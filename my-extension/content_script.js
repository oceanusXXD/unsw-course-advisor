...
try {
  ...
  // -------------------------
  // 以下是选课逻辑函数（从你原来 popup.js 的 startEnrollment / batchEnrollSameTerm 等迁移）
  // 已经适配为 content script 环境
  // -------------------------
  ...
  async function ensureTermActive(selector, timeout = 7000) {
    ...
    console.warn("⚠️ 切换学期超时，将在当前学期继续");
    return false;
  }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (
          ...
          console.log("🎉 注册成功！");
          // 尽量避免 document.open/write，改为把结果放在页面上或发送给 background
  ...
  async function batchEnrollSameTerm(courseCodes, termAlias) {
    ...
    console.log(`🚀 开始 ${termAlias} 学期批量选课: ${courseCodes.join(", ")}`);
    console.log("=".repeat(60));
    ...
    if (activeFormInfo.termInput) {
      ...
      while (Date.now() - start < 5000) {
        if (
          ...
          console.log("✅ active form 的 term 字段已更新");
          break;
        }
        await sleep(250);
    ...
    // 执行完整流程（搜索第一个课程 → 批量提交所有课程 → 确认）
    // 这里 courseCodes 是数组的课程 code 列表；我们依赖 window.__COURSE_ID_MAP 在 content 中可见
    ...
    if (attemptResult.success) {
      console.log(`✅ ${termAlias} 学期选课完成：成功`);
    } else {
      console.log(`❌ ${termAlias} 学期选课完成：失败`, attemptResult);
    }
  ...
  async function startEnrollment(courseList) {
    ...
    // 当成功时，将 completedTerms 放入 sessionStorage（供页面内脚本或后续检查使用）
