...
// - 假设 manifest 已声明 scripting, tabs, storage, activeTab 权限，以及 host_permissions 包含 my.unsw.edu.au
...
// -----------------------------
// 窗口管理（可选：如果你用 create popup window 的方式）
// -----------------------------
...
async function ensureContentScriptReady(tabId, timeout = 12000) {
  ...
  // 1) 动态注入 content_script.js（如果尚未注入）
  try {
...
async function processEnrollmentTerms(
  ...
  for (const term of termOrder) {
    ...
    try {
      ...
      // 失败时决定是否继续：这里选择继续下一个学期（如果你想中断，可在此处 break）
      // break;
    }
  }
